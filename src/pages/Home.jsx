...
const getRatingValue = (rating) => {
  ...
  return map[rating?.toLowerCase()] || 3;
};
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error("Erreur flux avis:", error);
      setLoadingReviews(false);
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleAvisSubmit = async (e) => {
    e.preventDefault();
    if (!avisData.name.trim() || !avisData.comment.trim()) return alert("Veuillez remplir les champs");
    ...
    try {
      ...
      await addDoc(collection(db, "reviews"), {
        name: avisData.name.trim(),
        comment: avisData.comment.trim(),
                      ...
                      <div className="flex gap-2 items-center bg-white/5 w-fit p-3 px-5 rounded-2xl border border-white/10">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <motion.button
                            ...
                            whileTap={{ scale: 0.9 }}
                            onClick={() => setAvisData({ ...avisData, rating: star })}
                          ...
                          >
                            <Star
                              ...
                              className={`transition-colors duration-200 ${
                                star <= (hoverRating || avisData.rating)
                                  ? "fill-eden-gold text-eden-gold"
                                  : "text-white/20"
                              }`}
                            />
                          </motion.button>
                        ))}
                        <span className="ml-4 text-eden-gold font-black-mango text-xl w-6 text-center">
                          {hoverRating || avisData.rating}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {loadingReviews ? (
              <div className="col-span-full py-20 flex justify-center">
                <Loader2 className="animate-spin text-eden-gold" />
              </div>
            ) : (
              reviews.map((rev, i) => {
                const ratingValue = getRatingValue(rev.rating);
                ...
                return (
                  <motion.div 
                    ...
                    className="bg-white/5 backdrop-blur-md p-8 md:p-10 rounded-[2.5rem] border border-white/10 flex flex-col h-full relative group hover:border-eden-gold/30 transition-colors"
                  >
                    ...
                    <div className="flex items-center gap-2 mb-6">
                      <div className="flex gap-1">
                        {[...Array(5)].map((_, star) => (
                          <Star
                            ...
                            className={
                              star < ratingValue
                                ? `fill-current ${getRatingColor(ratingValue)}`
                                : "text-white/10"
                            }
                          />
                        ))}
                      ...
                      <span className={`text-[10px] font-bold uppercase ${getRatingColor(ratingValue)}`}>
                        {typeof rev.rating === "number"
                          ? ["mauvais","mal","moyen","bien","très"][rev.rating - 1]
                          : rev.rating}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-4 pt-6 border-t border-white/10">
                      {rev.photoUrl ? (
                        ...
                        <div className="w-12 h-12 rounded-full bg-eden-gold/20 flex items-center justify-center text-eden-gold font-bold">
                          {rev.name?.charAt(0)}
                        </div>
                      )}
                      <div>
